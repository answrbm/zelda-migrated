...
public class ForrestScene extends ZeldaScene {
    ...
    public ForrestScene(Game game, String entrance) {
        ...
        Polygon upperTreeline = new Polygon(axpos, aypos, aypos.length);

        int[] cxpos = {203, 187, 180, 177, 328, 454, 506, 507, 361, 202};
        int[] cypos = {37, 16, 20, 4, 4, 4, 6, 39, 38, 37};

        Polygon upperTreeline2 = new Polygon(cxpos, cypos, cypos.length);
        ...
        Polygon leftTreeline = new Polygon(bxpos, bypos, bypos.length);

        int[] dxpos = {67, 60, 45, 71, 103, 120, 126, 142, 144, 5, 6, 66};
        int[] dypos = {398, 408, 429, 436, 435, 451, 482, 497, 509, 506, 398, 399};

        Polygon leftTreeline2 = new Polygon(dxpos, dypos, dypos.length);
        ...
        Polygon middleTreeline = new Polygon(expos, eypos, eypos.length);
        ...
        Polygon entreeWall1 = new Polygon(fxpos, fypos, fypos.length);

        int[] gxpos = {508, 483, 464, 462, 466, 477, 508, 508};
        int[] gypos = {345, 344, 329, 342, 363, 382, 385, 345};

        Polygon entreeWall2 = new Polygon(gxpos, gypos, gypos.length);
        ...
        Polygon lowerWall = new Polygon(hxpos, hypos, hypos.length);

        int[] ixpos = {507, 477, 474, 505, 507};
        int[] iypos = {390, 389, 424, 425, 391};

        Polygon deadtree1 = new Polygon(ixpos, iypos, iypos.length);

        int[] jxpos = {272, 306, 306, 273, 272};
        int[] jypos = {322, 322, 352, 353, 322};

        Polygon deadtree2 = new Polygon(jxpos, jypos, jypos.length);

        int[] kxpos = {153, 186, 186, 153, 153};
        int[] kypos = {478, 478, 508, 509, 481};

        Polygon deadtree3 = new Polygon(kxpos, kypos, kypos.length);
